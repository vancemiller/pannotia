...
__global__ void kernel(int timeinst, float* d_initvalu, float* d_finavalu, float* d_params,
    float* d_com) {
  ...
  // limit to useful threads
    ...
    if (tx == 0) {														// only 1 thread runs it, since its a sequential code
      ...
      kernel_cam(timeinst, d_initvalu, d_finavalu, valu_offset, d_params, params_offset, d_com,
          com_offset, CaDyad);
