...
template <bool isNP2> __device__ static void loadSharedChunkFromMem(uint32_t *s_data,
    const uint32_t *g_idata, int n, int baseIndex, int& ai, int& bi, int& mem_ai, int& mem_bi,
...
template <bool isNP2> __device__ static void storeSharedChunkToMem(uint32_t* g_odata,
    const uint32_t* s_data, int n, int ai, int bi, int mem_ai, int mem_bi, int bankOffsetA,
    int bankOffsetB) {
  __syncthreads();
...
template <bool storeSum> __device__ static void clearLastElement(uint32_t* s_data,
    uint32_t *g_blockSums, int blockIndex) {
  if (threadIdx.x == 0) {
...
__device__ static uint32_t buildSum(uint32_t *s_data) {
...
__device__ static void scanRootToLeaves(uint32_t *s_data, uint32_t stride) {
  uint32_t thid = threadIdx.x;
...
template <bool storeSum> __device__ static void prescanBlock(uint32_t *data, int blockIndex,
    uint32_t *blockSums) {
...
template <bool storeSum, bool isNP2> __global__ static void prescan(uint32_t *g_odata,
    const uint32_t *g_idata, uint32_t *g_blockSums, int n, int blockIndex, int baseIndex) {
  int ai, bi, mem_ai, mem_bi, bankOffsetA, bankOffsetB;
  extern __shared__ uint32_t s_data[];
  // load data into shared memory
  loadSharedChunkFromMem<isNP2>(s_data, g_idata, n, (baseIndex == 0) ? __mul24(blockIdx.x,
      (blockDim.x << 1)) : baseIndex, ai, bi, mem_ai, mem_bi, bankOffsetA, bankOffsetB);
  // scan the data in each block
  prescanBlock<storeSum>(s_data, blockIndex, g_blockSums);
  // write results to device memory
  storeSharedChunkToMem<isNP2>(g_odata, s_data, n, ai, bi, mem_ai, mem_bi, bankOffsetA,
      bankOffsetB);
}

__global__ static void uniformAdd(uint32_t *g_data, uint32_t *uniforms, int n, int blockOffset,
    int baseIndex) {
  __shared__ uint32_t uni;
  if (threadIdx.x == 0)
    uni = uniforms[blockIdx.x + blockOffset];
  uint32_t address = __mul24(blockIdx.x, (blockDim.x << 1)) + baseIndex + threadIdx.x;
  __syncthreads();
