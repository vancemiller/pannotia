...
void dump(float* variables, int nel, int nelr, bool unified) {
  float* h_variables;
  if (unified) {
    h_variables = variables;
  } else {
    h_variables = new float[nelr * NVAR];
...
__device__ __host__ inline void compute_flux_contribution(float& density, float3& momentum,
...
__global__ void cuda_compute_step_factor(int nelr, float* variables, float* areas,
    float* step_factors) {
  const int i = (blockDim.x * blockIdx.x + threadIdx.x);
...
__global__ void cuda_compute_flux(int nelr, int* elements_surrounding_elements, float* normals,
...
#pragma unroll
  for (j = 0; j < NNB; j++) {
    ...
    if (nb >= 0) {
      ...
      compute_flux_contribution(density_nb, momentum_nb, density_energy_nb, pressure_nb,
...
__global__ void cuda_time_step(int j, int nelr, float* old_variables, float* variables,
...
void time_step(int j, int nelr, float* old_variables, float* variables, float* step_factors,
    float* fluxes) {
...
int main(int argc, char** argv) {
  ...
  // set far field conditions and load them into constant memory on the gpu
  {
    ...
    h_ff_variable[VAR_DENSITY_ENERGY] = h_ff_variable[VAR_DENSITY]
      * (0.5f * (ff_speed * ff_speed)) + (ff_pressure / (GAMMA - 1.0f));
    ...
    checkCudaErrors(
        cudaMemcpyToSymbol(ff_flux_contribution_momentum_x, &h_ff_flux_contribution_momentum_x,
          sizeof(float3)));
    checkCudaErrors(
        cudaMemcpyToSymbol(ff_flux_contribution_momentum_y, &h_ff_flux_contribution_momentum_y,
          sizeof(float3)));
    checkCudaErrors(
        cudaMemcpyToSymbol(ff_flux_contribution_momentum_z, &h_ff_flux_contribution_momentum_z,
          sizeof(float3)));
  ...
  float* normals;
  {
    ...
    float* h_normals;
    if (unified) {
      checkCudaErrors(cudaMallocManaged(&h_elements_surrounding_elements, nelr * NNB * sizeof(int)));
  ...
  dump(variables, nel, nelr, unified);
