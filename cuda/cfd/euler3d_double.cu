...
void dump(double* variables, int nel, int nelr, bool unified)
{
  double* h_variables;
  if (unified) {
    h_variables = variables;
  } else {
    h_variables = new double[nelr * NVAR];
...
int main(int argc, char** argv)
{
  ...
  double* normals;
  {
    ...
    double* h_areas;
    if (unified) {
      checkCudaErrors(cudaMallocManaged(&h_areas, nelr * sizeof(double)));
    } else {
      h_areas = new double[nelr];
    }
    int* h_elements_surrounding_elements;
    double* h_normals;
    if (unified) {
      checkCudaErrors(cudaMallocManaged(&h_elements_surrounding_elements, nelr * NNB * sizeof(int)));
      checkCudaErrors(cudaMallocManaged(&h_normals, nelr * NDIM * NNB * sizeof(double)));
    } else {
      h_elements_surrounding_elements  = new int[nelr * NNB];
      h_normals = new double[nelr * NDIM * NNB];
    }
  ...
  double* variables;
  if (unified) {
    checkCudaErrors(cudaMallocManaged(&variables, nelr * NVAR * sizeof(double)));
  } else {
    variables = alloc<double>(nelr * NVAR);
  }
