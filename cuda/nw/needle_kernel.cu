...
__global__ void needle_cuda_shared_1(int* referrence, int* matrix_cuda, int cols, int penalty,
    int i, int block_width) {
  int bx = blockIdx.x;
  int tx = threadIdx.x;
  ...
  for (int m = 0; m < BLOCK_SIZE; m++) {
    ...
    if (tx <= m) {
      ...
      temp[t_index_y][t_index_x] = maximum(
          temp[t_index_y - 1][t_index_x - 1] + ref[t_index_y - 1][t_index_x - 1],
