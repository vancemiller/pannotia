...
#ifdef PROFILE
long long time_local_search;
long long time_speedy;
long long time_select_feasible;
...
float pFL(Points *points, int *feasible, int numfeasible, float z, long *k, int kmax, float cost,
    long iter, float e, int pid, pthread_barrier_t* barrier, bool unified) {
#ifdef ENABLE_THREADS
  pthread_barrier_wait(barrier);
#endif
  long i;
...
float pkmedian(Points *points, long kmin, long kmax, long* kfinal, int pid,
  ...
  while (1) {
    ...
    cost = pFL(points, feasible, numfeasible, z, &k, kmax, cost,
        (long) (ITER * kmax * log((float) kmax)), 0.1, pid, barrier, unified);
    ...
    if (((k <= (1.1) * kmax) && (k >= (0.9) * kmin)) || ((k <= kmax + 2) && (k >= kmin - 2))) {
      ...
      cost = pFL(points, feasible, numfeasible, z, &k, kmax, cost,
          (long) (ITER * kmax * log((float) kmax)), 0.001, pid, barrier, unified);
    }
...
void localSearch(Points* points, long kmin, long kmax, long* kfinal, bool unified) {
  ...
  TIMESTAMP(t2);
  time_local_search += ELAPSED(t1, t2);
