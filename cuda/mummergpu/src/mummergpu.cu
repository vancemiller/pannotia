...
void loadReferenceTexture(MatchContext* ctx, bool unified)
{
...
void loadReference(MatchContext* ctx, bool unified) {
  ...
  if (!ctx->on_cpu) {
    ...
    if (ref->tex_children_height)
    {
      ...
      fprintf(stderr, "after child_hist  ref->bytes_on_board:%lu\n", ref->bytes_on_board);
    ...
    bool unified)
{
  ...
  MatchInfo* M;
  if (unified) {
    cudaMallocManaged(&M, numMatches * sizeof(MatchInfo));
    memset(M, 0, numMatches * sizeof(MatchInfo));
  } else {
    M = (MatchInfo*)calloc(numMatches, sizeof(MatchInfo));
  }
  ...
  *nextqrychar = qrychar;
  ...
  if (unified) {
    cudaMallocManaged(alignments, alignmentOffset * sizeof(Alignment));
    memset(alignments, 0, alignmentOffset * sizeof(Alignment));
  } else {
...
void getExactAlignments(MatchContext * ctx, ReferencePage * page, bool on_cpu, bool unified)
{
...
int matchSubset(MatchContext* ctx,
    ReferencePage* page, bool unified)
{
  ...
  {
    getExactAlignments(ctx, page, false, unified);
  }
...
int matchQueriesToReferencePage(MatchContext* ctx, ReferencePage* page, bool unified)
{
...
int streamReferenceAgainstQueries(MatchContext* ctx, bool unified) {
  ...
  buildReferenceTexture(&(pages[0].ref),
      ...
      ctx->dotfilename,
      ctx->texfilename,
      &(ctx->statistics), unified);


  matchQueriesToReferencePage(ctx, &pages[0], unified);
  ...
  for (int i = 1; i < num_reference_pages - 1; ++i) {
    ...
    matchQueriesToReferencePage(ctx, &pages[i], unified);
  ...
  if (num_reference_pages > 1) {
    ...
    matchQueriesToReferencePage(ctx, &pages[last_page], unified);
