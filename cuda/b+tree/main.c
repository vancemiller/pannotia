    ...
    bool verbose, bool unified)
{
  ...
  printf("%d\n", malloc_size);
  if (unified) {
    cudaMallocManaged(&mem, malloc_size);
  } else {
    ...
    char** argv )
{
  ...
  for(cur_arg=1; cur_arg<argc; cur_arg++){
    ...
    } else if (strcmp(argv[cur_arg], "unified")==0) {
      printf("Unified enabled\n");
      unified = true;
  ...
  long mem_used = transform_to_cuda(root,0, unified);
      ...
      case 'k':
        {
          ...
          long *currKnode;
          if (unified) {
            cudaMalloc(&currKnode, count * sizeof(long));
          } else {
          ...
          record *ans;
          if (unified) {
            cudaMallocManaged(&ans, sizeof(record) * count);
          } else {
            ans = (record *)malloc(sizeof(record)*count);
          }
